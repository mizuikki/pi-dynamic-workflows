...
[![tests](https://img.shields.io/badge/tests-npm%20test-success)](#development)
...
Built for **codebase-wide audits, parallel review, large refactors, and cross-checked research** — anything one context window can't hold.
...
You get the permanent `workflow` tool plus commands for run control, settings,
and saved workflows. A supported Trellis project can conditionally add the
single `trellis_subagent` tool described below.

Workflow structured output is off by default. To opt in, add the literal boolean
below to `~/.pi/workflow-orchestrator/settings.json`:
...
`~/.pi/workflow-orchestrator/projects/<project-key>/settings.json` and wins over the global
...
- **One default Workflow Model** — `/workflow model` selects one model currently available and permitted by Pi's active session scope, plus an optional Pi-supported reasoning effort. An individual `agent()` call may explicitly override either axis for that call only.
...
- **Background by default** — the turn ends right away, a live "Workflows running" panel tracks runs, and each result is delivered back so the conversation auto-continues when it finishes. The panel is compact by default; `/workflow progress detailed` expands it inline to per-phase/per-agent rows with tokens, cost, and a live tok/s rate (so a stalled agent shows as 0 tok/s) — no need to open `/workflow`.
- **Interactive `/workflow` TUI** — drill runs → phases → agents → detail; inspect per-agent failures and compact subagent history; pause, stop, restart, and save runs from the keyboard.
...
- **Workflow intensity** — `/workflow intensity high|ultra` auto-arms thorough or exhaustive multi-agent workflows for substantive messages. It controls orchestration width and is separate from Pi model reasoning `effort`.
- **Saved & nested workflows** — save a run, execute it as `/workflow run @name`, or compose it from inside another workflow script.

## How it maps to Claude Code dynamic workflows

The same model — on Pi, plus the production pieces a real run needs:

| Claude Code dynamic workflows | Pi Workflow Orchestrator |
| --- | --- |
...
| Standing orchestration intensity | **`/workflow intensity ultra`** auto-arms an exhaustive workflow for every substantive message |
| — | **Git worktree isolation**, **real cost accounting**, and a **quality-pattern stdlib** |

## Commands

```text
/workflow                  open the interactive navigator (plain list in print mode)
/workflow status <id>      watch a run live; print its result when it finishes
/workflow run @name [--project|--global] [-- <args>]
                            execute the exact saved script through the normal manager path
/workflow list --saved [--project|--global|--all]
/workflow show @name [--project|--global]
/workflow save <name> [run-id] [--project|--global] [--replace]
/workflow delete @name [--project|--global]
/workflow pause|resume|stop|rm <id>
/workflow trigger off|on|status
                            persistently disable, restore, or inspect keyword triggering
/workflow trigger set <word>|reset
...
/workflow run <prompt>     force a dynamic workflow from <prompt> on demand — the explicit
                            twin of the keyword trigger. Works even when the keyword trigger
                            is off (/workflow trigger off); the run shows in the panel + /workflow.
/workflow progress compact|detailed|status
...
/workflow progress max <N> cap agents shown per phase in detailed mode (1-1000, default 8)
/workflow model           edit one global or project Workflow Model and its Pi-supported effort within Pi's active scope
/workflow prompt enable    enable the project-local main-agent prompt
/workflow prompt disable   disable the project-local main-agent prompt
/workflow prompt status    inspect project prompt metadata (path, state, size, and hash only)
/workflow intensity off|high|ultra
                            control standing orchestration width for substantive messages
...
Workflow run history and resume journals for every project are stored in one SQLite database at `~/.pi/workflow-orchestrator/workflow-orchestrator.sqlite3`. Runs remain isolated by a stable project key and Pi session ID. Listing and the live panel use summary records; private prompts, scripts, results, errors, and journals are loaded only for an authorized run detail or resume operation. The workflow home is user-only and the database file is created with user-only permissions on supported platforms.

Global settings, including the optional Workflow Model, live in `~/.pi/workflow-orchestrator/settings.json`. Saved workflow definitions and project settings are file-backed under `~/.pi/workflow-orchestrator/projects/<project>/`. State from the previous product root is not imported, read as a fallback, migrated, modified, or deleted. See [docs/storage.md](docs/storage.md) for the schema, durability, lease, backup, and recovery contract.
...
`loopUntilDry()`, `retry()`, and `gate()` are not gated.
...
file. Run `/workflow prompt enable` to persist the explicit project-local
...
`/workflow prompt status` is read-only and reports only the relative path,
...
| Global | What it does |
| --- | --- |
| `agent(prompt, opts)` | Spawn an isolated subagent. By default it returns final text; with the explicit structured-output opt-in, `opts.schema` returns a validated object. Recoverable failures return `null` with diagnostics in `/workflow`. |
...
For a Trellis `1.0.4` project, Pi Workflow Orchestrator can optionally:
...
Configured in `~/.pi/workflow-orchestrator/settings.json` (or a project override):
...
| `enabled` | Behavior |
| --- | --- |
| `"auto"` (default) | Enable context injection only when `<cwd>/.trellis/` exists and `.trellis/.version` is `1.0.4` |
| `"on"` | Attempt Trellis context resolution only for a `1.0.4` project |
...
`1.0.4` disables the adapter, registers no fallback tool, and emits a warning.
...
The canonical payload follows the Trellis `1.0.4` native renderer byte-for-byte:
...
Resolution order (fail closed and never scan another session pointer):

1. Prompt line `Active task: <path>`
2. Receiver-bound native `sessionManager.getSessionId()`, falling back to the
   receiver-bound transcript path; lossy session-ID normalization appends a
   hash of the original identity
3. Explicit child `TRELLIS_CONTEXT_ID` forwarding only; an ambient main-session
   value never overrides native identity
4. Read-only `python3 ./.trellis/scripts/task.py current --source`

The adapter never adopts a singleton process key and never scans unrelated
session files to guess the active task.
...
The "code mode for subagents" idea comes from Michael Livs' original
[pi-dynamic-workflows](https://github.com/Michaelliv/pi-dynamic-workflows) and
Anthropic's [dynamic workflows in Claude Code](https://claude.com/blog/introducing-dynamic-workflows-in-claude-code).
This project builds on it with explicit model routing, journaled resume,
git-worktree isolation, cost accounting, and an interactive TUI. The package
does not bundle fixed workflow scenarios or Web tools. See
[Architecture](docs/architecture.md) for the ownership and compatibility
boundaries.
