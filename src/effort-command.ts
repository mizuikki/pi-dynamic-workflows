 ...
 * auto-arms a workflow for substantive interactive messages, with orchestration-intensity
 ...
 * the script the model writes — so intensity is guidance plus the model setting
...
const ULTRA_DIRECTIVE =
  "Effort: ULTRA. Be exhaustive — fan out widely (more reviewers/judges, deeper loopUntilDry rounds, a completenessCheck at the end), and use the admitted Workflow Model for synthesis unless the user explicitly requests a temporary model/effort override. This can spend a lot of tokens quickly, so set explicit caps you're comfortable paying for (a generous but bounded tokenBudget and a high maxAgents) on the workflow tool call rather than leaving them unbounded.";
...
const ULTRA_TEXT_DIRECTIVE =
  "Effort: ULTRA. Be exhaustive — fan out widely with text-safe reviewers, deeper loopUntilDry rounds, and a final prose completeness pass; use retry() or gate() where useful, and use the admitted Workflow Model for synthesis unless the user explicitly requests a temporary override. This can spend a lot of tokens quickly, so set explicit caps you're comfortable paying for (a generous but bounded tokenBudget and a high maxAgents) on the workflow tool call. Workflow structured output is disabled, so do not rely on completenessCheck(), verify(), or judgePanel().";
...
export function registerEffortCommand(pi: ExtensionAPI, state: EffortState): void {
  ...
  pi.registerCommand("ultracode", {
    ...
    async handler(args: string, _ctx: ExtensionCommandContext) {
      ...
      state.level = "ultra";
      await say(
        "Ultracode ON — substantive messages now auto-arm an exhaustive workflow (wide fan-out and deeper synthesis). Use /ultracode off to stop.",
      );
    },
  });
}
